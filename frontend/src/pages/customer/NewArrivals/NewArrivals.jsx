          ...
          <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                {categoryOptions.map(cat => (
                  <button key={cat} onClick={() => setActiveCategory(cat)}
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all duration-200 whitespace-nowrap ${cat === activeCategory ? 'bg-[#198754] text-white shadow' : 'bg-[rgba(128,128,128,0.08)] text-[var(--text-secondary)] hover:bg-gray-200 dark:hover:bg-white/15'
                      }`}>
                    {cat}
                  </button>
                ))}
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              <div className="hidden sm:flex items-center bg-[rgba(128,128,128,0.08)] rounded-lg p-0.5">
                <button onClick={() => setViewMode('grid')} className={`p-1.5 rounded-md transition ${viewMode === 'grid' ? 'bg-white dark:bg-white/15 shadow text-[#198754]' : 'text-[var(--text-secondary)]'}`}>
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" /></svg>
                </button>
                <button onClick={() => setViewMode('list')} className={`p-1.5 rounded-md transition ${viewMode === 'list' ? 'bg-white dark:bg-white/15 shadow text-[#198754]' : 'text-[var(--text-secondary)]'}`}>
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
                </button>
              </div>
              <select value={sortBy} onChange={e => setSortBy(e.target.value)}
                className="text-sm font-medium text-[var(--text-primary)] bg-[rgba(128,128,128,0.08)] border-0 rounded-lg px-3 py-2 pr-8 focus:ring-2 focus:ring-[#198754] cursor-pointer">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <h2 className="text-xl font-extrabold text-[var(--text-primary)] mb-2">Could not load new arrivals</h2>
            <p className="text-[var(--text-secondary)] mb-6">{error}</p>
          </div>
        ) : products.length === 0 ? (
          <div className="text-center py-20">
            <span className="text-6xl mb-4 block">🆕</span>
            <h2 className="text-xl font-extrabold text-[var(--text-primary)] mb-2">No new arrivals in this category</h2>
            <p className="text-[var(--text-secondary)] mb-6">Check back soon — new products land every week!</p>
