    ...
    <div className={`w-full ${className}`}>
      ...
      <div className="relative">
        ...
        <input
          ...
          disabled={disabled}
          className={`
            w-full py-2.5 border rounded-sm text-sm bg-[var(--card-bg)] backdrop-blur-xl dark:text-white transition
            focus:outline-none focus:ring-2 focus:ring-[#2874F0] focus:border-[#2874F0]
            ${prefix ? 'pl-10' : 'pl-4'}
            ${suffix ? 'pr-10' : 'pr-4'}
            ${error ? 'border-red-500 focus:ring-red-400' : 'border-[var(--card-border)]'}
            ${disabled ? 'bg-[rgba(128,128,128,0.06)] text-gray-500 cursor-not-allowed' : ''}
          `}
          {...rest}
        />
